...
def round_number(n):
    k = 1 - int(math.log10(abs(n)))
    if n < 0:
        result = round(n, None if -n > 1 else k)
    else:
        result = round(n, None if n > 1 else k)
    return result 
...
def other_layers_derivative_value(centerpos1:float, centerpos2:float, L_value:float, 
        ...
        if derivative_L and conjugate:
                ...
                exp_term = cmath.exp(-PI * (centerpos1-centerpos2)**2 / (2 * PI * beta**2 - 1j * Lambda_ * L_value))
                term2 = (Lambda_ * L_value + 2j * PI * (beta + centerpos1 - centerpos2) * (beta - centerpos1 + centerpos2))
                denominator = ((2 * PI * beta**2 - 1j * Lambda_ * L_value)**2 *
                        cmath.sqrt(Lambda_ * L_value / (PI * beta**2) + 2j))
                return (term1 * exp_term * term2) / denominator        
        elif derivative_L and not conjugate:
                term = (-(1/2 + 1j/2) * Lambda_)*(2 * PI * (beta + centerpos1 - centerpos2) * (beta - centerpos1 + centerpos2) + 1j * Lambda_ * L_value)
                exp_term = cmath.exp(-PI * (centerpos1-centerpos2)**2 / (2 * PI * beta**2 + 1j * Lambda_ * L_value))
                denominator = ((2 * PI * beta**2 + 1j * Lambda_ * L_value)**2 *
                        cmath.sqrt(Lambda_ * L_value / (PI * beta**2) - 2j))
                ...
                term = (1 + 1j) * cmath.exp(-PI * (centerpos1-centerpos2)**2 / (2 * PI * beta**2 - 1j * Lambda_ * L_value))
                num = (-Lambda_**2 * L_value**2 + 2 * PI * beta**2 * (2 * PI * (centerpos1-centerpos2)**2 - 1j * Lambda_ * L_value))
                denominator = (beta * (2 * PI * beta**2 - 1j * Lambda_ * L_value)**2 *
                        cmath.sqrt(Lambda_ * L_value / (PI * beta**2) + 2j))
                return (term * num) / denominator
        else:
                term = (1 - 1j) * cmath.exp(-PI * (centerpos1-centerpos2)**2 / (2 * PI * beta**2 + 1j * Lambda_ * L_value))
                num = (Lambda_**2 * L_value**2 - 2 * PI * beta**2 * (2 * PI * (centerpos2-centerpos2)**2 + 1j * Lambda_ * L_value))
                denominator = (beta * (2 * PI * beta**2 + 1j * Lambda_ * L_value)**2 *
                        cmath.sqrt(Lambda_ * L_value / (PI * beta**2) - 2j))
                return (-term * num) / denominator
...
def sum_second(centerpos1:list, centerpos2:float, L_01:float, L_12:float, beta:list, conjugate:bool = False, derivative:bool = False, derivative_L:bool = True):
    ...
    for i in range(len(centerpos1)): 
        centerpos1[i] = round_number(centerpos1[i])
...
def forward_prop(centerpos1:list, centerpos2:list, delta_a_j:float, L_01:float, L_12:float, L_23:float, beta1:list, beta2:list, conjugate:bool = False, derivative:bool = False, derivative_L:bool = True):
    ...
    for i in range(-500,501): 
        ...
        for j in range(len(centerpos2)):# calculate ypred for each slits in second layer
            centerpos2[j] = round_number(centerpos2[j])
...
def derivative_for_L_01(centerpos:list, L_01):
    ...
    for i in range(len(centerpos)):
        centerpos[i] = round_number(centerpos[i])
...
def derivative_for_L_12_and_B_1(centerpos1:list, centerpos2:list, L_01:float, L_12:float, beta:list, L:bool = True):
    ...
    for i in range(len(centerpos2)):
        centerpos2[i] = round_number(centerpos2[i]) 
...
def result_of_derivative_b(centerpos1, centerpos2, L, beta):
     numerator = math.exp(-4*PI**2*beta**2*(centerpos1-centerpos2)**2/(4*PI**2*beta**4+Lambda_**2*L**2)) * (64*PI**5*beta**6*(centerpos1-centerpos2)**2 + 16*PI**3*beta**2*Lambda_**2*L**2*(beta+centerpos1-centerpos2)*(beta-centerpos1+centerpos2) + 4*PI*Lambda_**4*L**4) 
     denominator =  (beta * cmath.sqrt(Lambda_*L/beta**2 - 2*1j*PI) * cmath.sqrt(Lambda_*L/beta**2 + 2*1j*PI) * (4*PI**2*beta**4 + Lambda_**2*L**2)**2)
     expression = numerator / denominator

     return expression

def result_of_derivative_L01(centerpos1, L_01):

    numerator = -2 * math.sqrt(PI) * Lambda_**2 * L_01 * S**2 * math.exp(-4*PI**2*S**2*centerpos1**2 / (Lambda_**2*L_01**2 + 4*PI*S**4)) * (Lambda_**4*L_01**4 + 8*PI*Lambda_**2*L_01**2*S**2*(S**2 - PI*centerpos1**2) + 16*PI**2*S**6*(S**2 - 2*PI**2*centerpos1**2)) 
    denominator = (cmath.sqrt(S/(Lambda_*L_01 - 2*1j*PI*S**2)) * cmath.sqrt(S/(Lambda_*L_01 + 2*1j*PI*S**2)) * (Lambda_**2*L_01**2 + 4*PI*S**4)**2 * (Lambda_**2*L_01**2 + 4*PI**2*S**4)**2)
    expression = numerator / denominator

    return expression

def result_of_derivative_L(centerpos1, centerpos2, L, beta):
    numerator = (math.exp(-4*PI**2*beta**2*(centerpos1-centerpos2)**2/(4*PI**2*beta**4+Lambda_**2*L**2)) * (-8*PI**3*beta**2*Lambda_**2*L*(beta**2-2*(centerpos1-centerpos2)**2) - 2*PI*Lambda_**4*L**3))
    denominator = (cmath.sqrt(Lambda_*L/beta**2 - 2*1j*PI) * cmath.sqrt(Lambda_*L/beta**2 + 2*1j*PI) * (4*PI**2*beta**4 + Lambda_**2*L**2)**2)
